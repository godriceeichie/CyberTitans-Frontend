...
const ContactHero = () => {
  return (
    <Box as='section' py={'0'} px={{base: '6', md: '20'}} mt={'4.25rem'}>
        <Box w={'100%'} display={'flex'} flexDirection={{base: 'column', lg: 'row-reverse'}} gap={{base: '8', lg: '16' }} alignItems={{xl: 'center'}}>
            <Box w={{lg: '50%'}} >
                <Center>
            ...
            <Box flexGrow={'1'} bg={'white'} w={{lg: '50%'}} display={'flex'} flexDirection={'column'} gap={'8'}>
                <Flex direction={'column'} gap={{base: '2', xl: '0'}} alignItems={{base: 'center', lg: 'flex-start'}}>
                    <Heading as={'h2'} color={'brand.500'} fontSize={{base: '3xl', lg: '2.5rem'}} fontWeight={'bold'}>
                        Contact Us
                    </Heading>
                    <Text w={{xl: '80%'}} fontSize={{base: 'md'}} textAlign={{base: 'center', lg: 'left'}} color={'#999'} letterSpacing={'-0.0075rem'} lineHeight={'160%'} as={'p'}>
